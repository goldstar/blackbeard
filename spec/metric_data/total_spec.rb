    ...
    describe Total do
      ...
      describe "result_for_hour" do
        ...
        it "should return sum if metric called more than once" do
          ...
          expect(metric_data.result_for_hour(tz.now)).to eq({"total"=>6.0})
        end
      end
      ...
      describe "result_for_day" do
        ...
        context "result in db" do
          ...
          it "should return the result from db" do
            expect(metric_data.result_for_day(date)).to eq({"total"=>4.0})
          end
          it "should not remerge the results" do
            expect(metric_data).not_to receive(:generate_result_for_day).with(date)
        ...
        context "result not in db" do
          it "should merge hours" do
            expect(metric_data).to receive(:generate_result_for_day).with(date).and_return({"total" => "2"})
            expect(metric_data.result_for_day(date)).to eq({"total" => 2.0 })
          end
        end
      end
      ...
      describe "generate_result_for_day" do
        ...
        it "should sum the hours" do
          expect(metric_data.send(:generate_result_for_day, date)).to eq({"total" => 5.0})
        end

        it "should store the result if it's not today's result" do
          day_key = metric_data.send(:key_for_date, date)
          expect(db).to receive(:hash_multi_set).with(day_key, {"total" => 5})
