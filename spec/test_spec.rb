...
module Blackbeard
  RSpec.describe Test do
    ...
    describe "#probability_b_greater_than_a" do
      subject { test.probability_b_greater_than_a }
      before do
        allow(test).to receive(:number_of_successes_for_a).and_return(15)
        allow(test).to receive(:number_of_failures_for_a).and_return(9)
        allow(test).to receive(:number_of_successes_for_b).and_return(11)
        allow(test).to receive(:number_of_failures_for_b).and_return(12)
      end

      # Using Evan Miller's Julia implementation, we compute a test value:
      # julia> probability_B_beats_A(1+15, 1+9, 1+11, 1+12)
      let(:expected_probability) { 0.16096143114788317 }
